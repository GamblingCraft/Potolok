...
const _content = await usePageContent('natyazhnye-potolki-na-balkone')
...
const gallery = ref(await usePageGallery('natyazhnye-potolki-na-balkone'))
const portfolio = ref(await usePagePortfolio('natyazhnye-potolki-na-balkone'))
...
useHead({
  ...
  script: [{ type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Product', name: 'Натяжные потолки на балкон — утеплённый и холодный', description: 'Натяжные потолки на балкон в Иркутске от 159 ₽/м². Влагостойкие, работают при морозе. Монтаж за 1–2 часа. Гарантия 12 лет.', brand: { '@type': 'Brand', name: 'ПроПотолок' }, offers: { '@type': 'Offer', price: price.value, priceCurrency: 'RUB', availability: 'https://schema.org/InStock', seller: { '@type': 'LocalBusiness', name: 'ПроПотолок', address: { '@type': 'PostalAddress', addressLocality: 'Иркутск', addressCountry: 'RU' } } } }) }],
})

...
const _content = await usePageContent('natyazhnye-potolki-v-prihozhey')
...
const gallery = ref(await usePageGallery('natyazhnye-potolki-v-prihozhey'))
const portfolio = ref(await usePagePortfolio('natyazhnye-potolki-v-prihozhey'))
...
useHead({
  ...
  script: [{ type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Product', name: 'Натяжные потолки в прихожую', description: 'Натяжные потолки в прихожую в Иркутске. Теневой, световые линии, глянцевые.', brand: { '@type': 'Brand', name: 'ПроПотолок' }, offers: { '@type': 'Offer', price: price.value, priceCurrency: 'RUB', availability: 'https://schema.org/InStock', seller: { '@type': 'LocalBusiness', name: 'ПроПотолок', address: { '@type': 'PostalAddress', addressLocality: 'Иркутск', addressCountry: 'RU' } } } }) }],
})

...
const _content = await usePageContent('natyazhnye-potolki-v-ofise')
...
const gallery = ref(await usePageGallery('natyazhnye-potolki-v-ofise'))
const portfolio = ref(await usePagePortfolio('natyazhnye-potolki-v-ofise'))
...
useHead({
  ...
  script: [{ type: 'application/ld+json', innerHTML: JSON.stringify({ '@context': 'https://schema.org', '@type': 'Product', name: 'Натяжные потолки в офис — корпоративный стиль', description: 'Натяжные потолки в офис в Иркутске от 159 ₽/м². Световые линии, трековые системы, акустические. Монтаж без остановки работы. Гарантия 12 лет.', brand: { '@type': 'Brand', name: 'ПроПотолок' }, offers: { '@type': 'Offer', price: price.value, priceCurrency: 'RUB', availability: 'https://schema.org/InStock', seller: { '@type': 'LocalBusiness', name: 'ПроПотолок', address: { '@type': 'PostalAddress', addressLocality: 'Иркутск', addressCountry: 'RU' } } } }) }],
})

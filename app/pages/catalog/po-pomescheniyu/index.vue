  ...
  <div>
      ...
      <div class="container cl-hero__inner">
        <div>
          <div class="cl-breadcrumb">
            ...
            <span>По помещению</span>
          </div>
          <h1 class="cl-hero__title">Натяжные потолки по типу помещения</h1>
          <p class="cl-hero__desc">Подберём оптимальное решение с учётом влажности, освещения, высоты и стиля каждой комнаты</p>
        ...
        <div class="cl-hero__badge">
          <div class="cl-hero__badge-val">{{ pomeshcheniya.length }}</div>
          <div class="cl-hero__badge-label">типов помещений</div>
        </div>
      </div>
    </section>
    ...
    <section class="cl-section">
      <div class="container">
        <div class="cl-grid">
          <NuxtLink v-for="item in pomeshcheniya" :key="item.id" :to="item.slug" class="cl-card">
            ...
            <div class="cl-card__body">
              ...
              <div class="cl-card__price">от {{ item.price }} ₽/м²</div>
            </div>
        ...
        <div class="cl-cta">
          <div>
            <div class="cl-cta__title">Нужен потолок для всей квартиры?</div>
            <div class="cl-cta__desc">При заказе от 3 помещений — 3-й потолок в подарок. Рассчитайте стоимость онлайн или вызовите замерщика</div>
          </div>
        ...
        <div class="cl-seo">
          <h2>Натяжные потолки для разных помещений</h2>
          <p><strong>Гостиная и зал</strong> — парящие потолки, световые линии, многоуровневые конструкции для представительских интерьеров.</p>
          <p><strong>Спальня</strong> — матовые и сатиновые потолки с мягким освещением. Возможна скрытая подсветка над кроватью.</p>
          <p><strong>Кухня</strong> — влагостойкие ПВХ-полотна с точечными светильниками. Выдерживают пар и перепады температур.</p>
          <p><strong>Ванная</strong> — водонепроницаемые потолки, удерживают до 100 л/м² при затоплении.</p>
          <p><strong>Детская</strong> — тканевые гипоаллергенные полотна без ПВХ, сертифицированы для детских учреждений.</p>
        </div>
      </div>
    </section>
...
.cl-card { position: relative; display: block; border-radius: 14px; overflow: hidden; height: 200px; text-decoration: none; border: 2px solid transparent; transition: transform .2s, box-shadow .2s, border-color .2s; }
...
.cl-card__icon { color: var(--accent); margin-bottom: 4px; }
.cl-card__title { font-size: 16px; font-weight: 700; color: #fff; margin-bottom: 3px; }
...
@media (max-width: 480px) { .cl-grid { grid-template-columns: 1fr 1fr; } .cl-card { height: 150px; } .cl-cta { flex-direction: column; align-items: flex-start; } }
</style>

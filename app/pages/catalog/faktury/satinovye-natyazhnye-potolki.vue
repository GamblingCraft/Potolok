      ...
      <div class="container">
        <div class="cp-seo-inner">
          <div class="cp-seo-content">
            ...
            <p>Компания <strong>ПроПотолок</strong> реализует и устанавливает сатиновые натяжные потолки в Иркутске с 2009 года. Работаем с европейскими материалами, оплата после приёмки, даём письменную гарантию 12 лет.</p>
...
const _content = await usePageContent('satinovye-natyazhnye-potolki')
...
const gallery = ref(await usePageGallery('satinovye-natyazhnye-potolki'))
const portfolio = ref(await usePagePortfolio('satinovye-natyazhnye-potolki'))
const _prices = await useCatalogPrices()
const price = computed(() => _prices.value?.['satinovye'] ?? item.price)

const item = faktury.find(f => f.id === 'sat')!
const heroImg = usePageHero('satinovye-natyazhnye-potolki', item.img)

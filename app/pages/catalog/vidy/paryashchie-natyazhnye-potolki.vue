...
const _content = await usePageContent('paryashchie-natyazhnye-potolki')
...
const gallery = ref(await usePageGallery('paryashchie-natyazhnye-potolki'))
const portfolio = ref(await usePagePortfolio('paryashchie-natyazhnye-potolki'))
const _prices = await useCatalogPrices()
const price = computed(() => _prices.value?.['paryashchie'] ?? 350)
const priceExtra = computed(() => price.value - (_prices.value?.['base'] ?? 159))

const item = vidy.find(v => v.id === 'pary')!
const heroImg = usePageHero('paryashchie-natyazhnye-potolki', item.img)

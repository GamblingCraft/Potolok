...
const _content = await usePageContent('natyazhnye-potolki-so-svetovymi-liniyami')
...
const gallery = ref(await usePageGallery('natyazhnye-potolki-so-svetovymi-liniyami'))
const portfolio = ref(await usePagePortfolio('natyazhnye-potolki-so-svetovymi-liniyami'))
const _prices = await useCatalogPrices()
const price = computed(() => _prices.value?.['linii'] ?? 380)

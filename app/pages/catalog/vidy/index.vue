<template>
  <div>
    <section class="cl-hero">
      <div class="container cl-hero__inner">
        <div>
          ...
          <h1 class="cl-hero__title">Натяжные потолки по виду и технологии</h1>
          <p class="cl-hero__desc">Парящие ниши, подсветка, двухуровневые конструкции, световые линии и другие дизайнерские решения</p>
        ...
        <div class="cl-hero__badge">
          <div class="cl-hero__badge-val">{{ vidy.length }}</div>
          <div class="cl-hero__badge-label">технологий</div>
        </div>
      </div>
    </section>
    ...
    <section class="cl-section">
      <div class="container">
        <div class="cl-grid">
          <NuxtLink v-for="item in vidy" :key="item.id" :to="item.slug" class="cl-card">
            ...
            <div class="cl-card__body">
              ...
              <div class="cl-card__price" v-if="item.extra > 0">+{{ item.extra }} ₽/м² к базовой</div>
              <div class="cl-card__price" v-else>включено в стоимость</div>
            </div>
        ...
        <div class="cl-cta">
          <div>
            <div class="cl-cta__title">Хотите сложный потолок?</div>
            <div class="cl-cta__desc">Рассчитаем стоимость парящего, двухуровневого или потолка со световыми линиями — бесплатно</div>
          </div>
          <div class="cl-cta__btns">
            <button class="nav-btn cl-cta__btn" @click="callbackOpen = true">
              <Icon name="lucide:phone-call" size="16"/>Получить расчёт
            </button>
        ...
        <div class="cl-seo">
          <h2>Виды и технологии натяжных потолков</h2>
          <p><strong>Парящий потолок</strong> — световая ниша между полотном и стеной создаёт эффект левитирующего потолка. Современный тренд 2024–2026.</p>
          <p><strong>С подсветкой</strong> — точечные светильники, LED-ленты, трековые системы. Подбираем под освещение любой сложности.</p>
          <p><strong>Двухуровневые</strong> — зонирование пространства, скрытие коммуникаций, архитектурные формы в потолке.</p>
          <p><strong>Световые линии</strong> — встроенные LED-профили создают бесшовные светящиеся линии прямо в потолке.</p>
        </div>
      </div>
    </section>
...
.cl-card__badge { display: inline-block; background: var(--accent); color: var(--dark); font-size: 10px; font-weight: 800; padding: 2px 8px; border-radius: 5px; text-transform: uppercase; margin-bottom: 5px; }
...
.cl-cta__desc  { font-size: 14px; color: var(--gray); }

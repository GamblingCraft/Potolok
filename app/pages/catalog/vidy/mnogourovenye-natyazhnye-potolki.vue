      ...
      <div class="container">
        <div class="cp-seo-inner">
          <div class="cp-seo-content">
            ...
            <p>Компания <strong>ПроПотолок</strong> монтирует многоуровневые натяжные потолки в Иркутске. Индивидуальные проекты, оплата после приёмки, гарантия 12 лет.</p>
...
const _content = await usePageContent('mnogourovenye-natyazhnye-potolki')
...
const gallery = ref(await usePageGallery('mnogourovenye-natyazhnye-potolki'))
const portfolio = ref(await usePagePortfolio('mnogourovenye-natyazhnye-potolki'))
const _prices = await useCatalogPrices()
const price = computed(() => _prices.value?.['mnogourovenye'] ?? 600)
const priceExtra = computed(() => price.value - (_prices.value?.['base'] ?? 159))

const item = vidy.find(v => v.id === 'many')!
const heroImg = usePageHero('mnogourovenye-natyazhnye-potolki', item.img)
